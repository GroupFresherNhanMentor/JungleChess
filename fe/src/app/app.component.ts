...
import {
  EatenItem,
  EatenPopupComponent
} from './components/eaten-popup/eaten-popup.component';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  // --- Eaten-animal meme popups (player's piece captured) ---
  eatenPopups: EatenItem[] = [];
  private eatenSeq = 0;
  private eatenTimers: ReturnType<typeof setTimeout>[] = [];

  // --- Lobby ↔ Game "camera zoom" transition ---
  /** True while the zoom-into/out-of-the-forest transition is playing. */
  viewTransitioning = false;
  /** True → zoom OUT (game → lobby); false → zoom IN (lobby → game). */
  viewTransitionReverse = false;
  private viewTransitionTimer: ReturnType<typeof setTimeout> | null = null;

  // --- Screen shake on capture ---
  /** True while the whole app shakes (host class drives the CSS). */
  screenShaking = false;
  private shakeTimers: ReturnType<typeof setTimeout>[] = [];
  @HostBinding('class.screen-shake') get shakeClass(): boolean {
  ...
  public onBackToLobby(): void {
    // Zoom back out of the forest when returning to the lobby.
    this.playViewTransition(() => {
  ...
  /** Shake the whole screen for a beat after a capture. */
  ...
  private applyMove(from: Position, to: Position, outcome: 'attacker' | 'defender'): void {
    this.ngZone.run(() => {
      ...
      if (loser) {
        ...
        // Screen shake when a piece is captured (regardless of who ate it).
        this.shakeScreen();

        // Meme popup: the just-eaten animal pops out from the bottom-left
        // corner with a "?". Fire whenever the human player is a party to the
        // capture — the human's own animal eaten, or the human eating an AI
        // animal. In PVA the human is Blue (0); in PVP both sides are players.
        const humanInvolved =
          this.gameMode === 'PVA'
            ? loser.side === 0 || winnerSide === 0
            : true;
        if (humanInvolved) {
  ...
  /** Pop the just-eaten animal out from the bottom-left corner with a meme "?". */
  private launchEatenPopup(type: PieceType): void {
    this.ngZone.run(() => {
      const item: EatenItem = { id: ++this.eatenSeq, type };
      this.eatenPopups = [...this.eatenPopups, item];
      this.cdr.detectChanges();

      // Remove the popup once its animation finishes (matches the 2s CSS).
      const timer = setTimeout(() => {
        this.ngZone.run(() => {
          this.eatenPopups = this.eatenPopups.filter((p) => p.id !== item.id);
          this.cdr.detectChanges();
        });
      }, 2000);
      this.eatenTimers.push(timer);
    });
  }
}
